package de.xenyria.splatoon.game.equipment.gear.head;

import de.xenyria.splatoon.game.equipment.Brand;
import de.xenyria.splatoon.game.equipment.gear.SpecialEffect;
import org.bukkit.Color;
import org.bukkit.Material;

public class ArmorHelmetReplica extends HeadGear {

    public ArmorHelmetReplica() {
        super(9, Brand.CUTTLEGEAR, "Rüstungshelm-Replik", SpecialEffect.TENACITY, Material.LEATHER_HELMET, Color.BLACK, 3, 14000);
    }

    @Override
    public boolean useCustomModel() {
        return true;
    }

    @Override
    public short getCustomModelDamageValue() {
        return 7;
    }

    @Override
    public Material getCustomModelMaterial() {
        return Material.DIAMOND_HOE;
    }
}
